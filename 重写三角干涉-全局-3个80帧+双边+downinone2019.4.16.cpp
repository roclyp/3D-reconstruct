...
int main()
{
	...
	*outcloud_wholedown += *AllCloud.at(inicloudid);
	...
	int gapsize = para.savegap;
	for (int j = 1; j < ((para.circleEnd - para.circleBegin)*1.0 / gapsize) + 1; j++)
	{
		...
		for (int i = inicloudid; i < gapsize * j && i < endcloudid - 1; i = i + iterationnum, inicloudid += iterationnum)
		{
			cout << "-------------------------------------------------------------------" << endl;
			//获取点云文件1,2
