...
int main()//单循环1先和2配准，配准结果在和3配准，一直到最后
{
	...
	for (int i = 0; i < 4; i++)//for (int i = 0; i < PointAll.size(); i++)
	{
		DWORD PerStart = ::GetTickCount();
	...
	string saveName = "F:/1-DesktopFile_Pointget/1-DATA/xlr data/The_whole.ply";
	pcl::io::savePLYFile(saveName, *Cloud_Temp_Sort1);

	return 0;
}

int main_2()//分80组，先1和2配准，3和4配准，4和6配准。。。然后结果12和34配准，56与78配准。。一直到最后位置，存在多个循环
{
	...
	pcl::io::savePLYFile(saveName.str(), *Cloud_Temp_Last);
	return 0;
}