...
//获得旋转矩阵
void getrotateMatrix(string &rotateMatrixPath,Eigen::Matrix4d &rotateMatrix)
{
	ifstream rotatetxt;
...
void getspaceplane(TrianglePlane &mytriangles, const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud)
{
	for (int i = 0; i < mytriangles.triangle.polygons.size(); i++)
	{
		...
		pcl::PointCloud<PointXYZ>::Ptr pointgetcenter;
...
//判断是否干涉
bool isInterference(const spaceLine &Zer2P, const spacePlane &myPlane, const pcl::Vertices &tripolygons,
	const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud2, double &tempdis, PointXYZ &cloudProjected)
{
	...
	double area = caltriAngleArea(proPoint, cloud2->at(tripolygons.vertices.at(0)), cloud2->at(tripolygons.vertices.at(1)));
	area += caltriAngleArea(proPoint, cloud2->at(tripolygons.vertices.at(0)), cloud2->at(tripolygons.vertices.at(2)));
	area += caltriAngleArea(proPoint, cloud2->at(tripolygons.vertices.at(1)), cloud2->at(tripolygons.vertices.at(2)));
	//如果点在三角内，说明于三角面片干涉,那么记录该视角点到投影点的距离
	verticesPoint verPoint;
	verPoint.Point3vertices.push_back(cloud2->at(tripolygons.vertices.at(0)));
	verPoint.Point3vertices.push_back(cloud2->at(tripolygons.vertices.at(1)));
	verPoint.Point3vertices.push_back(cloud2->at(tripolygons.vertices.at(2)));
	if (area == verPoint.triarea)
	{
	...
	const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud2, pcl::PointXYZ &zerop, vector<interSituation> &interferenceCloud1)
{
	...
	for (int i = 0; i < cloud1->size(); i++)
	{
		...
		for (int j = 0; j < temptriangle.triangle.polygons.size(); j++)
		{
			...
			//如果是干涉情况，则保留最近的干涉
			if (isInterference(Zer2P, mytriangles2.myplaneall.at(j), mytriangles2.triangle.polygons.at(j), cloud2, tempdis, cloudProjected) == true)
			{
				if (tempdis <= mindis)
				{
					mindis = tempdis;
					...
					temp_inter.Verticespolygons = mytriangles2.triangle.polygons.at(j);
				}
...
int main()
{
	...
	getrotateMatrix(rotateMatrixPath,rotateMatrix);
	...
	greedy_triangle(cloud1, mytriangles1->triangle, KSearchnum, SearchRadius, mu, maxneibors, maxsurangle, minangle, maxangle, norConsis);
	greedy_triangle(cloud2, mytriangles2->triangle, KSearchnum, SearchRadius, mu, maxneibors, maxsurangle, minangle, maxangle, norConsis);

	getspaceplane(*mytriangles1, cloud1);
	getspaceplane(*mytriangles2, cloud2);
