...
int main()
{
	...
	*outcloud_wholedown += *AllCloud.at(inicloudid);
	...
	for (int j = 1; j < ((para.circleEnd - para.circleBegin)*1.0 / 80) + 1; j++)
	{
		...
		//初始化全局旋转矩阵
		*tempoutcloud_whole += *AllCloud.at(nextbeginid);
		*tempoutcloud_wholedown += *AllCloud.at(nextbeginid);
		pcl::transformPointCloud(*tempoutcloud_whole, *tempoutcloud_whole, global_transform);
		pcl::transformPointCloud(*tempoutcloud_wholedown, *tempoutcloud_wholedown, global_transform);
		...
		for (int i = inicloudid; i < 80 * j && i < endcloudid - 1; i = i + iterationnum, inicloudid += iterationnum)
		{
			...
			trianglulation(tempoutcloud_wholedown, meshcloud1, para);
			trianglulation(cloud2trans, meshcloud2, para);


			//先获取点云1，点云2三角面片对应的中心点云1，2；
			CloudCenVers cloudCenVer1(tempoutcloud_wholedown, meshcloud1);
			...
			//对点云2处理三角面片相交的情况
			TriCross(cloudCenVer2, cloud2trans, cloudCenVer1, tempoutcloud_wholedown, para);
			...
			DealCross2cLoud_new(cloudCenVer2, cloud2trans, tempoutcloud_wholedown, outcloud, cross2cloud, para);
			DealCross1cLoud_new(cloudCenVer2, cloud2trans, tempoutcloud_wholedown, outcloud, cross1cloud, para);
