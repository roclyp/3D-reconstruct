...
void getPara(mypara &mpara)
{
	...
	istringstream savegapstr(param["savegap"]);
	savegapstr >> mpara.savegap;
	...
	//MLS最小移动二值法
	mpara.PolynomialFit=param["PolynomialFit"];
	istringstream PolynomialOrderstr(param["PolynomialOrder"]),
		SearchRadiusmlsstr(param["SearchRadiusmls"]),
		leafsizemlsstr(param["leafsizemls"]);
	PolynomialOrderstr >> mpara.PolynomialOrder;
	SearchRadiusmlsstr>>mpara.SearchRadiusmls;
	leafsizemlsstr >> mpara.leafsizemls;
...
void MLS_Inpro(pcl::PointCloud<PointXYZRGB>::Ptr &cloudin, pcl::PointCloud<PointXYZRGB>::Ptr &cloudout,
	mypara &mpara)
{
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudtemp(new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::VoxelGrid<pcl::PointXYZRGB> sor;
	sor.setInputCloud(cloudin);
	sor.setLeafSize(mpara.leafsizemls, mpara.leafsizemls, mpara.leafsizemls);
	sor.filter(*cloudtemp);

	pcl::search::KdTree<pcl::PointXYZRGB>::Ptr search(new pcl::search::KdTree<pcl::PointXYZRGB>);
	pcl::MovingLeastSquares<pcl::PointXYZRGB, pcl::PointXYZRGB> mls;
	search->setInputCloud(cloudtemp);

	mls.setInputCloud(cloudtemp);
	if (mpara.PolynomialFit == "true")
		mls.setPolynomialFit(true);
	else
		mls.setPolynomialFit(false);
	mls.setPolynomialOrder(mpara.PolynomialOrder);//一般2-5
	mls.setSearchMethod(search);
	mls.setSearchRadius(mpara.SearchRadiusmls);//越大的话平滑力度越大
...
int main()
{
	...
	*outcloud_wholedown += *AllCloud.at(inicloudid);
	...
	for (int j = 1; j < ((para.circleEnd - para.circleBegin)*1.0 / para.savegap) + 1; j++)
	{
		...
		cout << "inicloudid: " << inicloudid << endl;
		cout << "para.savegap * j: " << para.savegap * j << " endcloudid - 1: " << endcloudid - 1 << endl;
		for (int i = inicloudid; i <= para.savegap * j && i < endcloudid - 1; i = i + iterationnum, inicloudid += iterationnum)
		{
			cout << "-------------------------------------------------------------------" << endl;
			//获取点云文件1,2
			...
			*outcloud += *cloud2transDelete;
			...
			for (int tempi = 0; tempi < cloudinsize; tempi++) {//cloudin->size()
				...
				if (kdtree1->nearestKSearch(outcloud->at(tempi), k + 1, kdpointID, kdpointDis)) {
					...
					/*	cloudin->at(i).r = 0;
						cloudin->at(i).g = 0;
						cloudin->at(i).b = 255;*/
					for (int tempj = 1; tempj <= k; tempj++)
					{
						//检查领域点
						tempkdcloud->push_back(outcloud->at(kdpointID[tempj]));
						tempnormals->push_back(normals->at(kdpointID[tempj]));
					}
			...
			//MLS处理降采样点云
			PointCloud<PointXYZRGB>::Ptr out_down_mls(new PointCloud<PointXYZRGB>);
			MLS_Inpro(outcloud, out_down_mls, para);


			*outcloud_whole += *outcloud;
			*outcloud_wholedown += *out_down_mls;
			//*outcloud_wholedown += *cloudout_down;
			*tempoutcloud_whole += *outcloud;
			*tempoutcloud_wholedown += *out_down_mls;
