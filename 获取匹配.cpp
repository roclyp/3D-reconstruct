...
int main()
{
	...
	/*	namedWindow("RGBpic", CV_WINDOW_NORMAL);
		...
		//		{
		//			saveline++;  
		//			string pathname = "C:/Users/Zhihong MA/Desktop/深度图像与RGB图像/";
		//			ostringstream DepthName;
		//			ostringstream RgbName;
		//			DepthName << pathname << "Depth_" << saveline << ".png";
		//			RgbName << pathname << "RGB_" << saveline << ".jpg";
		//			cv::imwrite(DepthName.str(), i_infrared);
		//			cv::imwrite(RgbName.str(), i_rgb);
		//		}
		//
			...
			for (int i = 0; i < 424 * 512; i++)
			{
				...
				if (p.X != -numeric_limits<float>::infinity() && p.Y != -numeric_limits<float>::infinity())
				{
					...
					if ((colorX >= 0 && colorX < 1920) && (colorY >= 0 && colorY < 1080))
					{
						...
						i_DepthToRgb.data[i * 4 + 3] = depthData[i];
					}
				}
			}
		}
