...
int main()
{
	...
	istringstream leafsizemlsstr(param["leafsizemls"]), PolynomialOrderstr(param["PolynomialOrder"]),
		SearchRadiusmlsstr(param["SearchRadiusmls"]);
	double leafsizemls, PolynomialOrder, SearchRadiusmls;
	PolynomialOrderstr >> PolynomialOrder;
	SearchRadiusmlsstr >> SearchRadiusmls;
	leafsizemlsstr >> leafsizemls;

	//初始化全局旋转矩阵
	global_transform = global_transform.Identity();
	...
	pcl::PointCloud<PointXYZRGB>::Ptr cloud_NoMls(new pcl::PointCloud<PointXYZRGB>);
	pcl::PointCloud<PointXYZRGB>::Ptr cloud_Mlstemp(new pcl::PointCloud<PointXYZRGB>);
	pcl::PointCloud<PointXYZRGB>::Ptr cloud_Mls(new pcl::PointCloud<PointXYZRGB>);
	pcl::copyPointCloud(*AllCloud.at(0), *cloud_NoMls);
	*cloud_Mlstemp = *AllCloud.at(0) + *AllCloud.at(1);

	pcl::VoxelGrid<pcl::PointXYZRGB> sorini;
	sorini.setInputCloud(cloud_Mlstemp);
	sorini.setLeafSize(leafsizemls, leafsizemls, leafsizemls);
	sorini.filter(*cloud_Mlstemp);

	pcl::search::KdTree<pcl::PointXYZRGB>::Ptr searchini(new pcl::search::KdTree<pcl::PointXYZRGB>);
	pcl::MovingLeastSquares<pcl::PointXYZRGB, pcl::PointXYZRGB> mlsini;
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr resultini(new pcl::PointCloud<pcl::PointXYZRGB>);
	searchini->setInputCloud(cloud_Mlstemp);

	mlsini.setInputCloud(cloud_Mlstemp);
	if (PolynomialFit == "true")
		mlsini.setPolynomialFit(true);
	else
		mlsini.setPolynomialFit(false);
	mlsini.setPolynomialOrder(PolynomialOrder);//一般2-5
	mlsini.setSearchMethod(searchini);
	mlsini.setSearchRadius(SearchRadiusmls);//越大的话平滑力度越大
	mlsini.process(*resultini);

	for (int i = 0; i < AllCloud.at(0)->size(); i++)
	{
		cloud_Mls->push_back(resultini->at(i));
	}

	//循环思路，cloud1和cloud2先进行配准和mls处理，mls处理后的第二帧点云再作为第三针的起始帧，然后在进行第三帧处理
	...
	{	
		...
		time_t fpfh_feature = GetTickCount();
		cout << "FPFH Compute Use time: " << ((fpfh_feature - t1)*1.0 / 1000) << " s" << endl;
		...
		time_t tfpfh = GetTickCount();
		cout << "FPFH Registeration Use time: " << ((tfpfh - fpfh_feature)*1.0 / 1000) << " s" << endl;
		...
		*tempfinal = *cloud1_target + *tricloud2xyzrgb;
		...
		sor.setLeafSize(leafsizemls, leafsizemls, leafsizemls);
	...
	viewer->addPointCloud(cloud_Mls, "cloud1_before_adjust");
	pcl::PolygonMesh mesh1, colormesh1;
	greedy_triangle(cloud_Mls, mesh1, paraall);
