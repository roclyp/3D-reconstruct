...
int main()
{
	...
	*outcloud_wholedown += *AllCloud.at(inicloudid);
	...
	for (int i = inicloudid; i < endcloudid; i = i + iterationnum)
	{
		...
		pcl::transformPointCloud(*cloud2_downbefore, *cloud2_downbefore, global_transform);
		//三角化以前都是对的
		...
		//trianglulation(cloud1, meshcloud3, para);
		...
		*AllCloud.at(i + iterationnum) += *downinverse;
		...
		cout << "Single frame size:" << outcloud->size() << "----";
		cout << "Single frame down size:" << cloudout_down->size() << endl;

		cout << "All frame size:" << outcloud_whole->size() << "----";
